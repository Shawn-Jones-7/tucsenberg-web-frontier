...
import { Options, run } from './ast-replace-magic-numbers';
...
async function runBatch() {
  ...
  const options: BatchOptions = {
    ...
    batchSize: parseInt(
      args.find((arg) => arg.startsWith('--batch-size='))?.split('=')[1] || '5',
    ),
  ...
  try {
    ...
    if (options.validateAfterEach && options.write && !options.dryRun) {
      ...
      try {
        ...
        const lintOutput = execSync(
          'pnpm run lint:check 2>&1 | grep "no-magic-numbers" | wc -l',
          {
            encoding: 'utf8',
            cwd: process.cwd(),
          },
    ...
    const totalReplacements = logs.reduce(
      (sum, log) => sum + log.changes.length,
      0,
    );
    ...
    if (totalReplacements > 0) {
      ...
      logs.forEach((log) => {
        ...
        if (Object.keys(log.imports.aliased).length > 0) {
          console.log(
            `    别名导入: ${Object.entries(log.imports.aliased)
              .map(([k, v]) => `${k} as ${v}`)
              .join(', ')}`,
          );
        }
