...
function validateConstantDefinitions() {
  ...
  for (const error of errors) {
    if (!availableConstants.includes(error.constantName)) {
      missingConstants.push({
        ...
        files: errors
          .filter((e) => e.constantName === error.constantName)
          .map((e) => e.file),
      });
    }
  }
  ...
  const uniqueMissing = missingConstants.reduce((acc, current) => {
    const existing = acc.find(
      (item) => item.constantName === current.constantName,
    );
    if (!existing) {
      acc.push(current);
    }
  ...
  uniqueMissing.forEach((missing) => {
    console.log(
      `  - ${missing.constantName} = ${missing.number}; // 用于 ${missing.files.length} 个文件`,
    );
  });
  return false;
}
...
function validateImports() {
  ...
  for (const [filePath, fileErrors] of fileGroups) {
    ...
    const missingImports = neededConstants.filter(
      (constant) => !importedConstants.includes(constant),
    );
...
function generateReport() {
  ...
  if (errors.length === 0) {
    ...
    report.next_steps.push(
      '🔧 运行 node scripts/safe-magic-numbers-fix.js 进行安全修复',
    );
...
function main() {
  ...
  try {
    ...
    console.log(
      `  剩余魔法数字错误: ${report.summary.total_magic_number_errors} 个`,
    );

    if (
      constantsValid &&
      importsValid &&
      report.summary.total_magic_number_errors === 0
    ) {
      console.log('\n🎉 所有验证通过！魔法数字修复完成！');
    } else {
