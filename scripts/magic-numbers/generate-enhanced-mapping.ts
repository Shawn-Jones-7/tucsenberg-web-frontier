...
async function generateEnhancedMapping() {
  ...
  console.log(`📊 原始CODEX映射: ${Object.keys(originalMapping).filter(k => !k.startsWith('_')).length} 个`);
  console.log(`📊 现有常量分析: ${analysisData._stats.uniqueValues} 个唯一数值`);
  ...
  for (const missingNumber of missingNumbers) {
    ...
    if (candidateMapping) {
      ...
      foundCount++;
      console.log(`  ✅ 找到缺失数字 ${missingNumber}: ${candidateMapping.export}`);
    } else {
  ...
  console.log(`\n💾 增强映射保存到: ${outputPath}`);
  console.log(`📊 总计映射: ${Object.keys(enhancedMapping).filter(k => !k.startsWith('_')).length} 个数值`);
  console.log(`✅ 缺失数字覆盖: ${foundCount}/${missingNumbers.length} (${Math.round(foundCount/missingNumbers.length*100)}%)`);
